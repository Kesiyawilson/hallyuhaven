...
const Add = ({ token }) => {
  ...
  const onSubmitHandler = async (e) => {
    ...
    try {
      ...
      formData.append("subcategory", subCategory); // ✅ fixed key
    ...
    <form onSubmit={onSubmitHandler} className="flex flex-col w-full items-start gap-3">
      ...
      <div>
        ...
        {["S", "M", "L", "XL", "XXL"].map((size) => (
          <span key={size} onClick={() =>
            setSizes((prev) =>
              prev.includes(size) ? prev.filter((s) => s !== size) : [...prev, size]
            )
          }>
            <p className={`${sizes.includes(size) ? "bg-pink-100" : "bg-slate-200"} px-3 py-1 cursor-pointer rounded border m-1`}>
              {size}
            </p>
          </span>
        ))}
